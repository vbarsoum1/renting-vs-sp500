...
import { getHistoricalDefaults, HISTORICAL_DATA } from './historicalData';
import { BrainCircuit, Loader2, TrendingUp, Home, DollarSign, Trophy, Sparkles, BookOpen } from 'lucide-react';
...
const App: React.FC = () => {
    ...
    const handleFetchDefaults = () => {
        // Use historical data directly - instant, no API call needed
        const historicalDefaults = getHistoricalDefaults();
        setParams(prev => ({ ...prev, ...historicalDefaults }));
    };
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div className="lg:col-span-4 space-y-6">
                        ...
                        <section className="bg-white rounded-2xl shadow-sm border border-slate-200">
                            <div className="bg-slate-50 px-6 py-3 border-b border-slate-100 flex items-center gap-2 rounded-t-2xl">
                            ...
                            <div className="p-6 grid grid-cols-1 gap-5">
                                <InputGroup
                                    ...
                                    onChange={(v) => updateParam('purchasePrice', v)}
                                    prefix="$"
                                    step={5000}
                                    historicalTooltip={{
                                        historicalValue: `$${HISTORICAL_DATA.purchasePrice.value.toLocaleString()}`,
                                        reason: HISTORICAL_DATA.purchasePrice.reason,
                                        source: HISTORICAL_DATA.purchasePrice.source
                                    }}
                                />
                                <div className="grid grid-cols-2 gap-4">
                                    <InputGroup
                                        ...
                                        suffix="%"
                                        historicalTooltip={{
                                            historicalValue: HISTORICAL_DATA.downPaymentPercent.value,
                                            reason: HISTORICAL_DATA.downPaymentPercent.reason,
                                            source: HISTORICAL_DATA.downPaymentPercent.source
                                        }}
                                    />
                                    <InputGroup
                                        label="Mortgage Rate"
                                        value={params.mortgageRate}
                                        onChange={(v) => updateParam('mortgageRate', v)}
                                        suffix="%"
                                        step={0.1}
                                        historicalTooltip={{
                                            historicalValue: HISTORICAL_DATA.mortgageRate.value,
                                            reason: HISTORICAL_DATA.mortgageRate.reason,
                                            source: HISTORICAL_DATA.mortgageRate.source
                                        }}
                                    />
                                </div>
                                <InputGroup
                                    ...
                                    onChange={(v) => updateParam('closingCostsPercent', v)}
                                    suffix="%"
                                    step={0.1}
                                    historicalTooltip={{
                                        historicalValue: HISTORICAL_DATA.closingCostsPercent.value,
                                        reason: HISTORICAL_DATA.closingCostsPercent.reason,
                                        source: HISTORICAL_DATA.closingCostsPercent.source
                                    }}
                                />
                                <InputGroup
                                    label="Amortization"
                                    ...
                                    suffix="Yrs"
                                    historicalTooltip={{
                                        historicalValue: HISTORICAL_DATA.amortizationYears.value,
                                        reason: HISTORICAL_DATA.amortizationYears.reason,
                                        source: HISTORICAL_DATA.amortizationYears.source
                                    }}
                                />
                            </div>
                        </section>
                        ...
                        <section className="bg-white rounded-2xl shadow-sm border border-slate-200">
                            ...
                            <div className="p-6 grid grid-cols-1 gap-5">
                                <InputGroup
                                    ...
                                    prefix="$"
                                    historicalTooltip={{
                                        historicalValue: `$${HISTORICAL_DATA.monthlyRent.value.toLocaleString()}`,
                                        reason: HISTORICAL_DATA.monthlyRent.reason,
                                        source: HISTORICAL_DATA.monthlyRent.source
                                    }}
                                />
                                <div className="grid grid-cols-2 gap-4">
                                    <InputGroup
                                        ...
                                        onChange={(v) => updateParam('vacancyRate', v)}
                                        suffix="%"
                                        historicalTooltip={{
                                            historicalValue: HISTORICAL_DATA.vacancyRate.value,
                                            reason: HISTORICAL_DATA.vacancyRate.reason,
                                            source: HISTORICAL_DATA.vacancyRate.source
                                        }}
                                    />
                                    <InputGroup
                                        label="Utilities (Landlord Paid)"
                                        value={params.utilitiesMonthly}
                                        onChange={(v) => updateParam('utilitiesMonthly', v)}
                                        prefix="$"
                                        historicalTooltip={{
                                            historicalValue: HISTORICAL_DATA.utilitiesMonthly.value,
                                            reason: HISTORICAL_DATA.utilitiesMonthly.reason,
                                            source: HISTORICAL_DATA.utilitiesMonthly.source
                                        }}
                                    />
                                </div>
                                <InputGroup
                                    ...
                                    suffix="% of Rent"
                                    historicalTooltip={{
                                        historicalValue: HISTORICAL_DATA.operatingExpensePercent.value,
                                        reason: HISTORICAL_DATA.operatingExpensePercent.reason,
                                        source: HISTORICAL_DATA.operatingExpensePercent.source
                                    }}
                                />
                            </div>
                        </section>
                        ...
                        <section className="bg-white rounded-2xl shadow-sm border border-slate-200">
                            <div className="bg-slate-50 px-6 py-3 border-b border-slate-100 flex items-center gap-2 rounded-t-2xl">
                                ...
                                <h3 className="font-semibold text-sm text-slate-700">Market Predictions</h3>
                            </div>
                            <div className="p-6 grid grid-cols-1 gap-5">
                                <div className="grid grid-cols-2 gap-4">
                                    <InputGroup
                                        ...
                                        step={0.1}
                                        historicalTooltip={{
                                            historicalValue: HISTORICAL_DATA.propertyAppreciation.value,
                                            reason: HISTORICAL_DATA.propertyAppreciation.reason,
                                            source: HISTORICAL_DATA.propertyAppreciation.source
                                        }}
                                    />
                                    <InputGroup
                                        label="Rent Increase"
                                        value={params.rentIncrease}
                                        onChange={(v) => updateParam('rentIncrease', v)}
                                        suffix="%"
                                        step={0.1}
                                        historicalTooltip={{
                                            historicalValue: HISTORICAL_DATA.rentIncrease.value,
                                            reason: HISTORICAL_DATA.rentIncrease.reason,
                                            source: HISTORICAL_DATA.rentIncrease.source
                                        }}
                                    />
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    <InputGroup
                                        ...
                                        onChange={(v) => updateParam('sp500Return', v)}
                                        suffix="%"
                                        step={0.1}
                                        historicalTooltip={{
                                            historicalValue: HISTORICAL_DATA.sp500Return.value,
                                            reason: HISTORICAL_DATA.sp500Return.reason,
                                            source: HISTORICAL_DATA.sp500Return.source
                                        }}
                                    />
                                    <InputGroup
                                        label="Exp. Inflation"
                                        ...
                                        step={0.1}
                                        historicalTooltip={{
                                            historicalValue: HISTORICAL_DATA.expenseInflation.value,
                                            reason: HISTORICAL_DATA.expenseInflation.reason,
                                            source: HISTORICAL_DATA.expenseInflation.source
                                        }}
                                    />
                                </div>
                            </div>
                    ...
                    <div className="lg:col-span-8 space-y-6">
                        ...
                        <div className="bg-white rounded-2xl shadow-sm border border-purple-100 overflow-hidden ring-1 ring-purple-50">
                            <div className="bg-gradient-to-r from-purple-50 to-white px-6 py-4 border-b border-purple-100 flex justify-between items-center">
                                ...
                                >
                                    {loadingAi ? <Loader2 className="animate-spin" size={14} /> : 'Tell Me What This Means'}
                                </button>
                            </div>
                                ...
                                {aiAnalysis ? (
                                    ...
                                    <div className="text-center py-6 px-4">
                                        ...
                                        <p className="text-slate-500 text-sm font-medium">
                                            Click the button above to see if this deal makes sense in the current market.
                                        </p>
                                    </div>
                                )}
                            </div>
